...
void busca(t_list **, int *, t_list **, t_list **); // Busca do elemento na lista
void deleta(t_list **, t_list **, t_list **); // Deleção de um elemento na lista
...
void deleta(t_list **cur, t_list **rem, t_list **h) {
  t_list *antes = *cur;
  // Faz o antes apontar para o anterior do elemento que vai ser removido
  while (antes->prox != *cur) {
    antes = antes->prox;
  }
  // Caso o removido seja a cabeça, passamos a cabeça para o próximo elemento para não perde-lá
  if (*cur == *h)
    *h = (*h)->prox;
...
void insere(t_list **h, t_list *new) {
  ...
  // Verificamos se é a posição da cabeça para relizarmos a inserção sem perde-lá
